...
ssWithMapWtCursor.each{svesWithMapWt ->
    ...
    svesWithoutMapWtSharingID.each{ssID, svesWithoutMapWt ->
        def rsHashesOfSvesWithMapWt = svesWithMapWtGroupedBySSID.get(ssID).collect{EVAObjectModelUtils.getClusteredVariantHash(it)}
        svesWithoutMapWt.each{sveWithoutMapWt ->
            if (rsHashesOfSvesWithMapWt.contains(EVAObjectModelUtils.getClusteredVariantHash(sveWithoutMapWt))) {
