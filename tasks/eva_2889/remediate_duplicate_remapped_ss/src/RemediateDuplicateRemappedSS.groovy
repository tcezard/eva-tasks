...
class RemediateDuplicateRemappedSS implements CommandLineRunner {
            ...
            sourceAssembly, svesWithDuplicates ->
                ...
                def svesToDeprecate = svesWithDuplicates.groupBy{sve -> sve.getAccession()}
                        .collect{accession, svesWithTheAccession ->
                            ...
                            return determineSSWithNRA(svesWithTheAccession, correspondingRemappingAttributes, correspondingEntriesInSourceAssembly)
                        }.flatten()
        }
    }

    // See https://docs.google.com/spreadsheets/d/1CW2V2uhsGDlTrDnu_NDpYgLUF-9LBSemDF61A7H7kYg/edit#gid=0
    static List<? extends SubmittedVariantEntity> determineSSWithNRA(svesWithTheAccession, correspondingRemappingAttributes, correspondingEntriesInSourceAssembly) {
        ...
        if (refAltPairsFromRemapping.size() == 0) {
            logger.error("No Ref/Alt remapping for accession ss$accession. This could be because this accession was not in the remapped VCF....")
        }
    ...
    static List<String[]> getPossibleRemappedAllelesForSVE(sveFromSourceAssembly, correspondingRemappingAttributes) {
        return correspondingRemappingAttributes.collect {remappingAttribute ->
            def (refFromSourceAssembly, altFromSourceAssembly) = [sveFromSourceAssembly.getReferenceAllele(),
                                                                  sveFromSourceAssembly.getAlternateAllele()]
    ...
    static List<String[]> normalizeAlleles(ref, alt) {
        // use VariantCoreFields as an indirect way to normalize alleles
        def dummyVariant = new VariantCoreFields("dummyChr", 1L, ref, alt)
...
class RemappingOutputParams implements Comparable<RemappingOutputParams> {
    ...
    RemappingOutputParams(String seq, Long accession, String strand, String referenceAlleleChange, boolean novelReferenceAllele) {
        this.seq = seq
