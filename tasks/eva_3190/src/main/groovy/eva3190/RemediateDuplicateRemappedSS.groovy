...
class RemediateDuplicateRemappedSS implements CommandLineRunner {
    ...
    static List<? extends SubmittedVariantEntity> determineSSWithInvalidRemaps(svesWithTheAccession, correspondingRemappingAttributes, correspondingEntriesInSourceAssembly) {
        ...
        if (svesNotGeneratedByRemapping.size() == svesWithTheAccession.size()) {
        ...
        if ((svesWithTheAccession.size() - svesNotGeneratedByRemapping.size()) > 1) {
            logger.error("Ambiguous remapping resolution: More than one remapped variant with accession ${accession} in assembly ${assemblyToRemediate} " +
                    "could be derived via remapping...")
        }
    ...
    static List<String[]> getPossibleRemappedAllelesForSVE(sveFromSourceAssembly, correspondingRemappingAttributes) {
        return correspondingRemappingAttributes.collect {remappingAttribute ->
            if (!Objects.equals(sveFromSourceAssembly.getClusteredVariantAccession(),
                    remappingAttribute.associatedRS)) return null
    ...
    static SubmittedVariantEntity guessRemappedVariantWithSameTypeForSVE(List<? extends SubmittedVariantEntity> remappedSVEsWithTheAccession,
        ...
        if (correspondingEntriesInSourceAssembly.size() == 0) return null
        ...
        if (correspondingEntriesInSourceAssembly.size() == 1) {
            ...
            def matchedVariant = remappedSVEsWithTheSameVariantType[0]
            if (sourceVariantType.equals(VariantType.SNV) &&
                    matchedVariant.getReferenceAllele() == sourceVariantSVE.getAlternateAllele() &&
                    matchedVariant.getAlternateAllele() == sourceVariantSVE.getReferenceAllele()) {
                logger.error("Flipped ref/alt in remapped assembly " +
                        "for accession ${sourceVariantSVE.getAccession()} " +
                        "in assembly ${sourceVariantSVE.getReferenceSequenceAccession()} when matching by type!!")
                return null
            }
    ...
    static boolean isCompatibleRemappedVariantType(VariantType sourceVariantType, VariantType remappedVariantType) {
        ...
        if ((sourceVariantType.equals(VariantType.INS) && remappedVariantType.equals(VariantType.INDEL)) ||
                ...
                (sourceVariantType.equals(VariantType.INDEL) && remappedVariantType.equals(VariantType.INS)) ||
                (sourceVariantType.equals(VariantType.INDEL) && remappedVariantType.equals(VariantType.DEL))) {
            return true
        }
        return false
    }
...
@EqualsAndHashCode
public class RemappingOutputParams implements Comparable<RemappingOutputParams> {
    String seq
    Long accession
    Long associatedRS
    String strand
    ...
    RemappingOutputParams(String seq, Long accession, Long associatedRS, String strand, String referenceAlleleChange,
                          boolean novelReferenceAllele) {
        this.seq = seq
        this.accession = accession
        this.associatedRS = associatedRS
        this.strand = strand
    ...
    static RemappingOutputParams parseFrom(Document remappingPropertiesDocument) {
        def rsFromDocument = remappingPropertiesDocument.get("RS")
        rsFromDocument = Objects.isNull(rsFromDocument) ? null: Long.parseLong(rsFromDocument.toString().replace("rs", ""))
    ...
    int compareTo(RemappingOutputParams other) {
        if(ObjectUtils.compare(this.seq, other.seq) == 0 &&
                ...
                ObjectUtils.compare(this.associatedRS, other.associatedRS) == 0 &&
