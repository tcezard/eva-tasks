...
def fix_discordant_variants(mongo_source, assembly, rs_file, batch_size=1000):
    ...
    with open(rs_file, 'r') as rs_file:
        while True:
            ...
            dbsnp_ss_variants, eva_ss_variants, all_ss_variants = get_ss_variants(mongo_source, rs_list)
            ...
            for rs in rs_list:
                ...
                if rs not in all_rs_variants:
                    ...
                    rs_without_map_weight = get_rs_without_map_weight(all_rs_variants[rs])
                    ...
                    non_remapped_ss = [ss for ss in all_ss_variants[rs] if
                                       'remappedFrom' not in ss and ss['seq'] == rs_variant['asm']]

                    if not non_remapped_ss:
                        logger.error(f"No original SS found for RS {rs}, /n {all_ss_variants[rs]}")
                        continue

                    if len(non_remapped_ss) > 1:
                        if not check_all_ss_has_same_info(non_remapped_ss):
                            logger.error(f"Not all original SS has same info : \n {non_remapped_ss}")
                            continue

                    if rs_variant['start'] == non_remapped_ss[0]['start']:
                        logger.error(f"RS's and original SS's Start matches. Nothing to do")
                        continue

                    rs_with_new_start = copy.copy(rs_variant)
                    rs_with_new_start['start'] = non_remapped_ss[0]['start']
                    ...
                    if result:
                        logger.warn(
                            f"Hash collision will occur for RS {rs_variant} with RS {variant_in_db['accession']}")
...
def update_ss_with_new_rs(old_rs, new_rs):
    ...
    dbsnp_sve_collection.update_many(filter_query, update_value)
    eva_sve_collection.update_many(filter_query, update_value)
...
def check_for_hash_collision(id):
    ...
    collision_records = find_documents(mongo_source, 'dbsnpClusteredVariantEntity', hash_collision_filter_criteria)
...
# def check_for_hash_collision(id_list):
#     hash_collision_filter_criteria = {'_id': {'$in': [x for x in id_list.keys()]}}
#     collision_records = find_documents(mongo_source, 'dbsnpClusteredVariantEntity', hash_collision_filter_criteria)
#
#     if len(collision_records) > 0:
#         for record in collision_records:
#             if record['_id'] in id_list:
#                 rs = id_list[record['_id']]['accession']
#                 logger.error(f"Hash Collision will occur for RS : {rs} with {record['accession']} : "
#                              f"\nRS:       {id_list[record['_id']]} \nRS in DB: {record}")
