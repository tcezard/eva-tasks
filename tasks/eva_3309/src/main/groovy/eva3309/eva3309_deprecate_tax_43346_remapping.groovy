...
class Deprecate43346TaxRemappedSS {
    ...
    static void deprecateSS(EVADatabaseEnvironment dbEnv, Collection<? extends SubmittedVariantEntity> svesToDeprecate) {
        ...
        def svDeprecationWriter = new SubmittedVariantDeprecationWriter(inputParameters.assemblyAccession,
                ...
                "EVA3309", "Variant incorrectly remapped to the wrong assembly for tax 43346")
...
// This script deprecates variants incorrectly remapped in the bison bison species (tax 43346)
...
def impactedAssembly = "GCA_002263795.2"
def impactedStudy = "PRJEB40501"
...
def tax43346VariantCursor = [sveClass, dbsnpSveClass].collect {
    new RetryableBatchingCursor(where("seq").is(impactedAssembly)
            .and("study").is(impactedStudy).and("remappedFrom").exists(true), prodEnv.mongoTemplate, it)
}
tax43346VariantCursor.each{it.each{sves ->
    // Deprecate matching SS with no duplicates in the remapped assembly
    Deprecate43346TaxRemappedSS.deprecateSS(prodEnv, sves)
}}
// Delete all operations involving the taxonomy and study in GCA_002263795.2
def tax43346SubmittedOpCursor = [svoeClass, dbsnpSvoeClass].collect {
    new RetryableBatchingCursor<>(where("inactiveObjects.seq").is(impactedAssembly)
            .and("inactiveObjects.study").is(impactedStudy), prodEnv.mongoTemplate, it)
}
int numSubmittedOpsRemoved = 0
tax43346SubmittedOpCursor.each {currCursor -> currCursor.each{svoes ->
