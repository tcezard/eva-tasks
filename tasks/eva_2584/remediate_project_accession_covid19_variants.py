...
def replace_with_correct_project_accession(mongo_source, assembly_accession, incorrect_prj_accession,
                                          correct_prj_accession, num_variants_to_replace):
    sve_collection = mongo_source.mongo_handle[mongo_source.db_name]["submittedVariantEntity"]
    filter_criteria = {'seq': assembly_accession, 'study': incorrect_prj_accession}
    ...
    try:
        for variant in cursor:
            ...
            variant['study'] = correct_prj_accession
...
def main():
    parser = argparse.ArgumentParser(
        description='Replace incorrect project accession in a given assembly with the correct one', add_help=False)
    ...
    parser.add_argument("--incorrect-prj-accession", help="Study accession (ex: PRJEB29734)", required=True)
    parser.add_argument("--correct-prj-accession", help="Study accession (ex: PRJEB29734)", required=True)
    ...
    replace_with_correct_project_accession(mongo_source, args.assembly_accession, args.incorrect_prj_accession,
                                args.correct_prj_accession, args.num_variants_to_replace)
