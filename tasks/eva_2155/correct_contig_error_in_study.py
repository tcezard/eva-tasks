...
def replace_with_correct_contig(mongo_source):
    ...
    filter_criteria = {'seq': 'GCA_000298735.1', 'study': 'PRJEB33693', 'contig': '-'}
...
def delete_variants_wront_contig(mongo_source):
    sve_collection = mongo_source.mongo_handle[mongo_source.db_name]["submittedVariantEntity"]
    filter_criteria = {'seq': 'GCA_000298735.1', 'study': 'PRJEB33693', 'contig': 'OARMT'}
    cursor = sve_collection.find(filter_criteria, no_cursor_timeout=True)
    drop_statements = []
...
def correct(mongo_source):
    num_variants_updated = replace_with_correct_contig(mongo_source)
    num_variants_dropped = delete_variants_wront_contig(mongo_source)
    return num_variants_updated, num_variants_dropped


def main():
    parser = argparse.ArgumentParser(description='Correct contig error in study PRJEB33693', add_help=False)
